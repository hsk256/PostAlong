...
public class TimeUtils {
     ...
     * 本地时间转换为后台保存时间
     *
     * @param localeTime 本地格式时间
     * @return 后台保存格式的时间：年月日时分秒
     */
    public static String localeToSaveTime(String localeTime) {
        if (localeTime != null) {
            try {
                return BaseUtils.newSimpleDateFormat().format(BaseUtils.newLocaleDateFormat().parse(localeTime));
    ...
    public static String getFormatDate1() {
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat(Constants.TIME_FORMAT_SAVE_ALL);
        Date date = new Date();
        return simpleDateFormat.format(date);
    }
    ...
    public static String getWeekDay(String inputDate) {
        String weekStrArr1[] = {"星期日", "星期一", "星期二", "星期三", "星期四", "星期五",
                "星期六"};
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        Date date = null;
        try {
